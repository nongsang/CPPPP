...
// 델리게이트가 아니라 이벤트로 구현하는 법이다.
// CallbackArg 최상위 클래스를 System.EventArgs라는 이름으로 전부 바꾼다.

class PrimeCallbackArg : EventArgs    // 소수값을 전달받아 인스턴스를 생성하는 클래스
{
    public int Prime;
...
{
    //public delegate void PrimeDelegate(object sender, EventArgs arg); // 콜백을 위한 델리게이트 정의
    ...
    public event EventHandler PrimeGenerated;   // 델리게이트에서 이벤트로 생성

    // PrimeDelegate인 callbacks를 event 선언된 PrimeGenerated로 변경
    public void Run(int limit)      // 양수를 넣으면 그 수까지 소수를 찾는다.
    {
        for (int i = 2; i <= limit; ++i)
        {
            ...
            {
                // 콜백을 발생시킨 측의 인스턴스와 발견된 소수를 콜백 메서드에 전달
                PrimeGenerated(this, new PrimeCallbackArg(i));       // 여기서 소수값을 전달받자마자 델리게이트에 연결된 모든 메소드를 실행
            }
        }
    }

    // 델리게이트 소수생성기에서 썼던 코드와 같음
    private bool IsPrime(int candidate)     // 소수인지 판별하는 메소드
    {
        if ((candidate & 1) == 0)     // 들어온 값이 짝수중에서
            return candidate == 2;  // 2가 맞다면 소수이므로 true를 반환, 아니라면 소수가 아니므로 false
...
namespace CPPPP
{
    class Event_Prime_Generator
    {
        // 콜백으로 등록될 메소드 1
        static void PrintPrime(object sender, EventArgs arg)  // 소수값을 전부 출력하는 메소드
        {
        ...
        // 콜백으로 등록될 메소드 2
        static void SumPrime(object sender, EventArgs arg)    // 소수값을 전부 더하는 메소드
        {
